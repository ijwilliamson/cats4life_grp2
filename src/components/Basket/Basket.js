...
const Basket = (props) => {
  ...
  const buildBasketJSX = () => {
    if (
      typeof props.basketState === "undefined" ||
      props.basketState.length === 0
    ) {
      // if the basketState does not exist then return no items

      return (
        <div className="basketItems">
          <p className="empty-basket-text">There are no cats in your basket</p>
        </div>
      );
    }
      ...
      return (
        <div className="basketItems" key={index}>
          <div className="basket-photo">
            <img src={item.url} width="100px" alt={item.name} />
          </div>
    ...
    <div className={overlayClass()}>
      <div className="basket">
        <div className="basketItems basketHeader">
          <div className="title-photo">Photograph</div>
          <div className="title-name">Name</div>
          <div className="title-price">Price</div>
        </div>

        <div className="allItems">{buildBasketJSX()}</div>

        <div className="basketItems total">
          <div className="total-photo"></div>
          <div className="total-name"></div>
          <div className="total-price">Total: £{calcTotal()}</div>
        </div>
        ...
        <div className="footer">
          ...
          <div>
            <button className="cat-description-btn" onClick={props.toggle}>
              Back to store
            </button>
            <button
